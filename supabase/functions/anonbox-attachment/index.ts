...
const ALLOWED = new Set([
  ...
  "audio/mpeg", "audio/mp4", "audio/webm", "audio/ogg", "audio/wav", "audio/x-wav", "video/mp4",
]);
...
async function participant(conversationId: string, deviceId: string, userId: string | null): Promise<Participant | null> {
  const { data: rows, error } = await admin.from("anonbox_messages_v2")
    ...
    .eq("conversation_id", conversationId).is("deleted_at", null).order("created_at", { ascending: true }).limit(500);
  if (error || !rows?.length) return null;
  const boxId = String(rows[0].box_id);
  const { data: box } = await admin.from("anonbox_boxes").select("owner_id").eq("id", boxId).maybeSingle();
  if (!box) return null;
  ...
  if (userId && userId === ownerId) {
    ...
    return { actor:"owner", mode:"owner", boxId, ownerId, userId, conversationId,
  ...
  if (userId) {
    ...
    if (profileRow) {
      const { data: p } = await admin.from("anonbox_profiles").select("pseudonym,avatar_path").eq("user_id", userId).maybeSingle();
      return { actor:"visitor", mode:"profile", boxId, ownerId, userId, conversationId,
  ...
  if (deviceId.length >= 8 && deviceId.length <= 200) {
    ...
    if (anonRow) return { actor:"visitor", mode:"anonymous", boxId, ownerId, userId:null, conversationId, fingerprint:fp,
      pseudonym:anonRow.sender_pseudonym_snapshot || null, avatarPath:anonRow.sender_avatar_path_snapshot || null };
  }
  return null;
}

async function publicParticipant(slug: string, modeRaw: string, deviceId: string, userId: string | null): Promise<Participant | null> {
  const mode = modeRaw === "profile" ? "profile" : "anonymous";
  if (deviceId.length < 8 || deviceId.length > 200) return null;
  const { data: box } = await admin.from("anonbox_boxes").select("id,owner_id,is_open,allow_anonymous,allow_profile_messages").eq("slug", slug).maybeSingle();
  if (!box || !box.is_open) return null;
  if (mode === "anonymous" && !box.allow_anonymous) return null;
  if (mode === "profile" && !box.allow_profile_messages) return null;
  const boxId = String(box.id), ownerId = String(box.owner_id), fp = await sha256Hex(`${boxId}:${deviceId}`);
  let conversationId: string | null = null, pseudonym: string | null = null, avatarPath: string | null = null;
  if (mode === "profile") {
    if (!userId) return null;
    const { data: p } = await admin.from("anonbox_profiles").select("pseudonym,avatar_path").eq("user_id", userId).maybeSingle();
    if (!p) return null;
    pseudonym = p.pseudonym || "Profil"; avatarPath = p.avatar_path || null;
    const { data: prior } = await admin.from("anonbox_messages_v2").select("conversation_id").eq("box_id", boxId).eq("direction","visitor").eq("sender_mode","profile").eq("sender_user_id",userId).order("created_at",{ascending:false}).limit(1).maybeSingle();
    conversationId = prior?.conversation_id || null;
  } else {
    const { data: prior } = await admin.from("anonbox_messages_v2").select("conversation_id").eq("box_id", boxId).eq("direction","visitor").eq("sender_mode","anonymous").eq("sender_fingerprint",fp).order("created_at",{ascending:false}).limit(1).maybeSingle();
    conversationId = prior?.conversation_id || null;
  }
  return { actor:"visitor", mode, boxId, ownerId, userId:mode === "profile" ? userId : null, fingerprint:fp,
    pseudonym, avatarPath, conversationId:conversationId || crypto.randomUUID() };
}

async function rateLimited(p: Participant) {
  const since = new Date(Date.now() - 10 * 60 * 1000).toISOString();
  let q = admin.from("anonbox_messages_v2").select("id", { count:"exact", head:true }).eq("box_id",p.boxId).gte("created_at",since);
  if (p.actor === "owner") q = q.eq("direction","owner");
  else q = q.eq("direction","visitor").eq("sender_fingerprint",p.fingerprint);
  const { count, error } = await q;
  return !!error || (count || 0) >= (p.actor === "owner" ? 30 : 5);
}

async function upload(req: Request) {
  let form: FormData;
  try { form = await req.formData(); } catch { return json(req,{error:"Formulaire invalide."},400); }
  let conversationId = String(form.get("conversation_id") || "").trim();
  const slug = String(form.get("slug") || "").trim().toLowerCase();
  const mode = String(form.get("mode") || "anonymous");
  ...
  const viewOnce = String(form.get("view_once") || "false") === "true";
  const durationRaw = Number(form.get("voice_duration_ms") || 0);
  const voiceDurationMs = Number.isFinite(durationRaw) && durationRaw > 0 ? Math.min(300000, Math.round(durationRaw)) : null;
  ...
  if (conversationId) {
    ...
    p = await participant(conversationId,deviceId,userId);
  } else if (slug) {
    p = await publicParticipant(slug,mode,deviceId,userId);
    conversationId = p?.conversationId || "";
  }
  if (!p || !conversationId) return json(req,{error:"Conversation introuvable."},403);
  if (p.mode === "anonymous") {
    if (anonPseudonym) {
  ...
  if (await rateLimited(p)) return json(req,{error:"Trop de messages envoyés. Réessaie un peu plus tard."},429);
  ...
  const actualBody = caption || (isVoice ? "🎤 Message vocal" : `📎 ${originalName}`);
  const placeholder = isVoice ? "👁️ Message vocal à vue unique" : "👁️ Pièce jointe à vue unique";
  const row: any = {
    box_id:p.boxId, conversation_id:conversationId, direction:p.actor === "owner" ? "owner" : "visitor",
    body:viewOnce ? placeholder : actualBody, sender_mode:p.mode, sender_user_id:p.userId,
    sender_pseudonym_snapshot:p.pseudonym, sender_avatar_path_snapshot:p.avatarPath, sender_fingerprint:p.fingerprint,
    read_at:null, reply_to_id:replyTo, view_once:viewOnce, voice_duration_ms:voiceDurationMs,
  };
  if (!viewOnce) Object.assign(row,{attachment_id:id,attachment_name:originalName,attachment_mime:mime,attachment_size:file.size,attachment_path:path});
  ...
  if (viewOnce) {
    const { error: payloadError } = await admin.from("anonbox_view_once_payloads").insert({
      message_id:inserted.id, body:actualBody, attachment_id:id, attachment_name:originalName,
      attachment_mime:mime, attachment_size:file.size, attachment_path:path, voice_duration_ms:voiceDurationMs
    });
    if (payloadError) {
      await admin.from("anonbox_messages_v2").delete().eq("id",inserted.id);
      await admin.storage.from(BUCKET).remove([path]);
      return json(req,{error:"Échec de création du message à vue unique."},500);
    }
  }
  return json(req,{ok:true,message_id:inserted.id,conversation_id:conversationId,attachment_id:id,view_once:viewOnce,voice:isVoice},201);
}
...
async function sign(req: Request, body: any) {
  ...
  const { data: msg } = await admin.from("anonbox_messages_v2").select("id,conversation_id,attachment_path,attachment_name,attachment_mime,view_once").eq("id",messageId).is("deleted_at",null).maybeSingle();
  if (!msg || msg.view_once || !msg.attachment_path) return json(req,{error:"Pièce jointe introuvable."},404);
  const userId = await requestUser(req);
  const p = await participant(String(msg.conversation_id),deviceId,userId);
  if (!p) return json(req,{error:"Accès refusé."},403);
  const { data, error } = await admin.storage.from(BUCKET).createSignedUrl(String(msg.attachment_path),300,{download:false});
  if (error || !data?.signedUrl) return json(req,{error:"Lien temporaire indisponible."},500);
