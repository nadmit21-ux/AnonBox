...
const admin = createClient(SUPABASE_URL, adminKey, { auth: { persistSession: false } });
...
async function firebaseAccessToken(sa: ServiceAccount): Promise<string> {
  ...
  if (!tokenResponse.ok || !tokenData.access_token) throw new Error(`OAuth Firebase refusé (${tokenResponse.status})`);
...
Deno.serve(async (req: Request) => {
  ...
  } catch { return json({ error: "Invalid Firebase server credentials" }, 503); }

  const { data: msg, error: msgError } = await admin.from("anonbox_messages_v2").select("id,box_id,conversation_id,direction,sender_mode,created_at").eq("id", messageId).maybeSingle();
  if (msgError || !msg) return json({ error: "Message not found" }, 404);
  if (msg.direction !== "visitor") return json({ ok: true, skipped: "not_visitor" });
  if (Date.now() - new Date(msg.created_at).getTime() > 10 * 60 * 1000) return json({ ok: true, skipped: "too_old" });

  const { data: box, error: boxError } = await admin.from("anonbox_boxes").select("owner_id").eq("id", msg.box_id).single();
  if (boxError || !box) return json({ error: "Owner not found" }, 404);
  const { data: devices, error: deviceError } = await admin.from("anonbox_push_tokens").select("id,token").eq("user_id", box.owner_id).eq("enabled", true).eq("platform", "android");
  if (deviceError) return json({ error: "Push devices unavailable" }, 500);
  if (!devices?.length) return json({ ok: true, skipped: "no_devices" });

  const { error: claimError } = await admin.from("anonbox_push_deliveries").insert({ message_id: messageId });
  if (claimError) {
  ...
  try {
    ...
    for (const device of devices) {
      const fcm = await fetch(`https://fcm.googleapis.com/v1/projects/${encodeURIComponent(sa.project_id)}/messages:send`, {
        method: "POST",
        headers: { Authorization: `Bearer ${accessToken}`, "Content-Type": "application/json" },
        body: JSON.stringify({ message: {
          token: device.token,
          notification: { title: "Nouveau message AnonBox", body: "Tu as reçu un nouveau message." },
          data: { conversation_id: String(msg.conversation_id), message_id: String(msg.id) },
          android: { priority: "high", notification: { channel_id: "anonbox_messages", sound: "default" } },
        } }),
      });
      if (fcm.ok) { sent++; continue; }
      const err = await fcm.text();
      failures.push(`${fcm.status}:${err.slice(0, 300)}`);
      if (fcm.status === 404 || err.includes("UNREGISTERED") || err.includes("registration-token-not-registered")) invalidTokenIds.push(device.id);
    }
    if (invalidTokenIds.length) await admin.from("anonbox_push_tokens").update({ enabled: false }).in("id", invalidTokenIds);
    await admin.from("anonbox_push_deliveries").update({ sent_at: sent > 0 ? new Date().toISOString() : null, delivered_count: sent, last_error: failures.length ? failures.join(" | ").slice(0, 2000) : null }).eq("message_id", messageId);
